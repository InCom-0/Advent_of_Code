...
std::pair<size_t, size_t> day17_0(std::string dataFile) {
    ...
    // DATA PREP
    auto xCoords_string = incom::aoc::parseInputUsingCTRE::processFileRPT(dataFile, xCtre);
    auto yCoords_string = incom::aoc::parseInputUsingCTRE::processFileRPT(dataFile, yCtre);
    ...
    for (auto &line : xCoords_string) {
        if (line.front().contains("..")) {
            std::vector<std::string> split =
                incom::aoc::parseInputUsingCTRE::processOneLineRPToneVect(line.front(), dCtre);
