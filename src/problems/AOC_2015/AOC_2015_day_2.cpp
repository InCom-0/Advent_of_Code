...
int day2_1(std::string dataFile) {
    ...
    while (std::getline(iStream, oneStr)) {
        ...
        small  = INT_MAX;
        accu  += 2 * flux::ref(lwh)
                        .adjacent_map<2>([](auto &&a, auto &&b) { return a * b; })
                        .take_while([&](auto &&a) {
...
int day2_2(std::string dataFile) {
    ...
    while (std::getline(iStream, oneStr)) {
        ...
        accu += flux::ref(lwh).fold_first([](auto &&acc, auto &&b) { return acc * b; }).value();

        flux::sort(lwh, std::less());
        accu += 2 * flux::ref(lwh).take(2).sum();
