...
namespace AOC2015 {
    int day9_1(std::string dataFile) {
        ...
        for (int i = 0; i < lookUp.size(); ++i) {
                ...
                for (auto &option: arp[lvlIdx]) {
                    for (int j = 0; j < lookUp.size(); ++j) {
                        if (option.first[j] == 0) {
                            ...
                            arp[lvlIdx + 1].at(toInsert) = std::min(arp[lvlIdx + 1].at(toInsert),
            ...
            minSoFar = std::min(minSoFar, std::min_element(arp.back().begin(), arp.back().end(),
    ...
    int day9_2(std::string dataFile) {
        ...
        for (int i = 0; i < lookUp.size(); ++i) {
            ...
            minSoFar = std::max(minSoFar, std::max_element(arp.back().begin(), arp.back().end(),
                                                           [](auto &&a, auto &&b) { return a.second < b.second; })->second);
        }
