...
Design pattern inspired by 'deducing this' feature of C++23, but not actually using it since GCC 13 doesnt support it
atm ... Technically the following solution should be identical for this purpouse (although not possible to do everything
'deducing this' enables). Essentially passing a lambda to itself as 'auto deduced' parameter.
*/
long long day24_1(std::string dataFile, int numberOfGroups) {
    ...
    std::vector<std::vector<long long>> groups(
        numberOfGroups, std::vector<long long>()); // first group is special ie. needs to have smalest number of items
    ...
    then it rolls back up and exits back into group1Selector (it only need to find one such solution, hence reducing
    complexity to manageable level)

    group1Selector is also a recursive lambda that finds all the valid 'group 0' combinations of the lowest number of
    members.
    */
        ...
        for (int i = startPos; i < inputsIntoNextStage[currentLevel].size(); ++i) {
            ...
            if (groupSums[currentLevel] == weightPortion) {
                if (currentLevel == numberOfGroups - 2) { // Must be valid answer
                    ...
                    inputsIntoNextStage[currentLevel + 1] =
                        flux::zip(flux::ref(inputsIntoNextStage[currentLevel]), flux::ref(masks[currentLevel]))
                            .filter(filter1)
                            .map(map1)
                            .to<std::vector<int>>();
                    currentLevel++;
            ...
            else if (groupSums[currentLevel] < weightPortion) {
                self(self, i + 1); // same level, need more numbers to satisfy weight
            }
            if (groupSums[currentLevel] == weightPortion && ans.back() == groups[0]) {
                return;                   // only need to find 1 valid combination for each valid groups[0]
            }
    ...
    auto group1Selector = [&](auto const &self, int startPos) -> void {
        for (int i = startPos; i < inputNumbers.size(); ++i) {
            ...
            if (groupSums[0] == weightPortion && minGroup1 >= groups[0].size()) {
                ...
                inputsIntoNextStage[currentLevel + 1] =
                    flux::zip(flux::ref(inputNumbers), flux::ref(masks[currentLevel]))
                        .filter(filter1)
                        .map(map1)
                        .to<std::vector<int>>();
