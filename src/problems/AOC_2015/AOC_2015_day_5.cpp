...
int day5_1(std::string dataFile) {
    ...
    while (std::getline(iStream, oneStr)) {
        ...
        auto vvv = flux::ref(oneStr).slide(2).fold(
            [&](auto &&acc, auto &&a) {
        ...
        auto vowelCount = flux::ref(oneStr).fold(
            [](auto &&acc, auto &&a) {
...
int day5_2(std::string dataFile) {
    ...
    while (std::getline(iStream, oneStr)) {
        bool isThis = true;
        flux::ref(oneStr).slide(3).for_each_while([&](auto &&a) {
        ...
        isThis = true;
        flux::ref(oneStr).slide(2).for_each_while([&](auto &&a) {
            tempStr = flux::to<std::string>(a);
