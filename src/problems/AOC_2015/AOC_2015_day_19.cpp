...
int day19_1(std::string dataFile, std::string input) {
    ...
    auto add2Left = flux::ref(changes).filter([](auto &&a) { return a.second.ends_with(a.first); }).map([](auto &&a) {
        return std::pair<std::string, std::string>(a.first, a.second.substr(0, a.second.size() - a.first.size()));
    });
