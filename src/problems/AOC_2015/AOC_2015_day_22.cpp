...
Technically extensible design capable of easily accomodating all sort of actions or effects the player or the boss can
take.
...
(ie. mana below recharge cost => known finite & comparativelly small number of possibilities which will always be the
same.)

Note: Basically designed as an excercise to familirize myself with how one could create a barebones simple 'event' based
system
*/
int day22_1(int Ehp, int Edmg, int hp, int mana) {
    ...
    auto playerRound = [&]() {
        ...
        if (testConditions()) {
            for (auto nextAction : actions) { nextAction(); }
        }
        for (auto event : eventTable.insideTable[curState.round]) { event(-1); }
        curState.round--;
    };
    auto bossAction = [&](int antiReverse) { curState.myHP -= (curState.bossDMG - curState.myArmor) * antiReverse; };
    auto bossRound  = [&]() -> void {
        curState.round++;
        eventTable.checkAndAddUntil(curState.round);
        for (auto event : eventTable.insideTable[curState.round]) { event(1); }
