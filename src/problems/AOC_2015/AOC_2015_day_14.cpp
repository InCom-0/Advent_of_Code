...
int day14_1(std::string dataFile, int raceDuration) {
    ...
    while (raceCounter <= raceDuration) {
        for (int i = 0; i < names.size(); ++i) {
            ...
            if (modu <= flightDur[i] && modu > 0) {
                calculatedTravel[i] += speed[i] * 1; // So that it is obvious that we mean 1 second here.
            }
        }
        raceCounter++;
    }
...
int day14_2(std::string dataFile, int raceDuration) {
    ...
    while (raceCounter <= raceDuration) {
        ...
        for (int i = 0; i < names.size(); ++i) {
            ...
            else if (maxDist == multiplier * speed[i] * flightDur[i] + modu * speed[i]) { maxPoss.push_back(i); }
        }
        for (auto &idxToIncrement : maxPoss) { calculatedTravel[idxToIncrement]++; }
