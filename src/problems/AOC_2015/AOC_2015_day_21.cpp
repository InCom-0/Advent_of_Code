...
namespace AOC2015 {
    ...
    int day21_1(int hp, int dmg, int armor, int myHP, std::string dataFile) {
        ...
        for (int i = 0; i < ringL.size(); ++i) {
            for (int j = i + 1; j < ringL.size(); ++j) {
                toSort.push_back(equip{
                    ringL[i].cost + ringL[j].cost,
                    ringL[i].damage + ringL[j].damage,
                    ringL[i].armor + ringL[j].armor,
                    ringL[i].name + "_" + ringL[j].name
                });
            }
        }
        ...
        toSort = flux::ref(toSort).chunk_by([](auto &&a, auto &&b) {
                    return a.damage * 64 + a.armor == b.damage * 64 + b.armor;
                })
                .map([](auto &&a) {
                    return *std::min_element(a.begin(), a.end(), [](auto &&yyy, auto &&zzz) {
                        return yyy.cost < zzz.cost;
                    });
                })
                .to<std::vector<equip> >();
        ...
        auto ans = flux::ref(toSort).chunk_by([](auto &&a, auto &&b) {
                    return a.damage * 64 + a.armor == b.damage * 64 + b.armor;
                })
                .map([](auto &&a) {
                    return *std::min_element(a.begin(), a.end(), [](auto &&yyy, auto &&zzz) {
    ...
    int day21_2(int hp, int dmg, int armor, int myHP, std::string dataFile) {
        ...
        toSort = flux::ref(toSort).chunk_by([](auto &&a, auto &&b) {
                ...
                .map([](auto &&a) {
                    return *std::max_element(a.begin(), a.end(), [](auto &&yyy, auto &&zzz) {
                        return yyy.cost < zzz.cost;
                    });
                })
                .to<std::vector<equip> >();
