...
long long day11_0(std::string dataFile, int const repeats) {
    ...
    auto input    = incom::aoc::parseInputUsingCTRE::processFileRPT(dataFile, any_ctre).front();

    std::vector<size_t> data;
    for (auto const &number : input) { data.push_back(std::stoull(number)); }

    ankerl::unordered_dense::map<std::pair<size_t, int>, size_t, incstd::hashing::XXH3Hasher> pastResults;
    ...
    auto rec_solve = [&](this auto const &self, size_t const numOnStone, int const blinkRemaining) {
        ...
        else if ((hlpr = incstd::numeric::get_numOfDigits(numOnStone)) % 2 == 0) {
