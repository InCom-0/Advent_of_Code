...
long long day3_1(std::string dataFile) {
    ...
    auto input    = incom::aoc::parseInputUsingCTRE::processFile(dataFile, any_ctre);

    std::string data;
    for (auto const &line : input.front()) { data += line; }

    auto mul_ctre = ctre::search<R"(mul\(\d{1,3},\d{1,3}\))">;
    auto parsed   = incom::aoc::parseInputUsingCTRE::processOneLineRPToneVect(data, mul_ctre);

    auto d_ctre = ctre::search<R"(\d+)">;

    long long res = 0;
    for (auto &parsedLine : parsed) {
        auto twoNumbers  = incom::aoc::parseInputUsingCTRE::processOneLineRPToneVect(parsedLine, d_ctre);
...
long long day3_2(std::string dataFile) {
    ...
    for (auto const &splt : splitByDo) {
        ...
        auto parsed      = incom::aoc::parseInputUsingCTRE::processOneLineRPToneVect(doInThisStr, mul_ctre);
