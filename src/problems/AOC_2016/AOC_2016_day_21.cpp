...
namespace AOC2016 {
    ...
    std::string day21_1(std::string dataFile, std::string &&input) {
        ...
        std::vector<std::variant<SwapPoss,SwapLetter,RotR,RotL,RotBOP,Reverse_X_through_Y,MovePos_x_To_y>> actionsToTake;
        ...
        while (std::getline(iStream, oneStr)) {
            ...
            else if (oneStr.starts_with("swap letter ")) {
                actionsToTake.push_back(SwapLetter{oneStr[12],oneStr.back()});
            }
            else if (oneStr.starts_with("rotate right ")) {
                actionsToTake.push_back(RotR{0,0, oneStr[13]-48});
            }
            else if (oneStr.starts_with("rotate left ")) {
                actionsToTake.push_back(RotL{0,0, oneStr[12]-48});
            }
            else if (oneStr.starts_with("rotate based ")) {
                actionsToTake.push_back(RotBOP{oneStr.back()});
            }
        ...
        for (auto &oneAction: actionsToTake) {
            std::visit(overloaded{
                                    ...
                                    [&] (const SwapLetter& a) -> void {std::swap(input[input.find(a.letter_1)], input[input.find(a.letter_2)]);},
                                    [&] (const RotR& a) -> void {std::rotate(input.rbegin(),input.rbegin()+a.pos_1 % input.size(),input.rend());},
                                    [&] (const RotL& a) -> void {std::rotate(input.begin(),input.begin()+a.pos_1 % input.size(),input.end());},
                                    [&] (const RotBOP& a) -> void {std::rotate(
                                                        input.rbegin(),
                                                        input.rbegin()+(input.find(a.letter_1) > 3 ? input.find(a.letter_1) + 2 : input.find(a.letter_1) + 1) % input.size(),
                                                        input.rend());
                                                        },
                                    [&] (const Reverse_X_through_Y& a) -> void {std::reverse(input.begin()+a.pos_1, input.begin()+a.pos_2+1);},
        ...
        */
        for (auto &oneAction: std::views::reverse(actionsToTake)) {
            std::visit(overloaded{
                                    [&] (const SwapPoss& a) -> void {std::swap(input[a.pos_1], input[a.pos_2]);},
                                    [&] (const SwapLetter& a) -> void {std::swap(input[input.find(a.letter_1)], input[input.find(a.letter_2)]);},
                                    [&] (const RotR& a) -> void {std::rotate(input.begin(),input.begin()+a.pos_1 % input.size(),input.end());},
                                    [&] (const RotL& a) -> void {std::rotate(input.rbegin(),input.rbegin()+a.pos_1 % input.size(),input.rend());},
                                    [&] (const RotBOP& a) -> void {
