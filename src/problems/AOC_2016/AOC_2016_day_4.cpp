...
namespace AOC2016 {
    ...
    auto res = flux::zip(flux::ref(vectOfRoomMaps), flux::ref(inp[2]), flux::ref(inp[1]).map([] (auto&& g) {return std::stoi(g);}))
                                        .filter([] (auto&& a) {
                                            return flux::zip(flux::ref(std::get<0>(a)), flux::ref(std::get<1>(a)))
                                                        .all([] (auto&& aaa) {
...
int day4_2(std::string dataFile) {
    ...
    std::vector<std::vector<std::pair<char,int>>> vectOfRoomMaps;
    for (auto roomStr: inp[0]) {
        std::unordered_map<char, int> tempMap;      // might be better to use a find on vector<pair<char,int>> ... max element size 32 finding single char
        for (auto& charInRoomStr: roomStr) {
            if (charInRoomStr == '-') continue;
            if (tempMap.contains(charInRoomStr)) tempMap.at(charInRoomStr)++;
            else tempMap.emplace(charInRoomStr, 1);
    ...
    auto res = flux::zip(flux::ref(inp[0]), flux::ref(inp[1]).map([] (auto&& g) {return std::stoi(g);}), flux::ref(inp[2]), flux::ref(vectOfRoomMaps))
                        .filter([] (auto&& a) {
                            return flux::zip(flux::ref(std::get<3>(a)), flux::ref(std::get<2>(a)))
                                        .all([] (auto&& aaa) {
                                            return aaa.first.first == aaa.second;
                                        });
                        })
