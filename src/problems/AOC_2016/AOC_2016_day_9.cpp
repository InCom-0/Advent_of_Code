...
int day9_1(std::string &&dataFile) {
    ...
    while (rgxResult) {
        ...
        for (int i = 0; i < multiple; ++i) { newStr.append(rgxResult.get_end_position(), bg); }
...
long long day9_2(std::string &&dataFile) {
    ...
    */
    auto lam = [&](this auto &self, int multiplicator, std::string::iterator &&beg,
                   std::string::iterator &&end) -> long long {
