...
std::vector<int> day23_1(std::string dataFile, int regA, int regB, int regC, int regD) {
    ...
    std::vector<
        std::variant<cpy, cpyInput, inc, dec, jnz, jnzInput, jnzOffsetTarget, jnzCondOffsetTarget, tgl, skip1, skip2>>
        instrVect;
    ...
    while (std::getline(iStream, oneStr)) {
        ...
        if (tmpStr == "cpy") {
            ...
            if (re) {
                instrVect.push_back(
                    cpyInput{re.to_number(), INT_MIN, nullptr,
            ...
            instrVect.push_back(
                dec{INT_MIN, INT_MIN, nullptr, mapping.at(getWord_ctre(bg, oneStr.end()).to_string()[0])});
        }
        else if (tmpStr == "inc") {
            instrVect.push_back(
                inc{INT_MIN, INT_MIN, nullptr, mapping.at(getWord_ctre(bg, oneStr.end()).to_string()[0])});
        }
        else if (tmpStr == "jnz") {
            ...
            if (re2) {
                ...
                if (re) {
                    instrVect.push_back(jnzCondOffsetTarget{INT_MIN, INT_MIN, mapping.at(re2.to_string()[0]),
                                                            mapping.at(re.to_string()[0])});
                }
                else {
                    instrVect.push_back(jnz{INT_MIN, getNumber_ctre(re2.get_end_position(), oneStr.end()).to_number(),
                ...
                if (re2) {
                    instrVect.push_back(
                        jnzOffsetTarget{re.to_number(), INT_MIN, nullptr, mapping.at(re2.to_string()[0])});
                }
                else {
                    instrVect.push_back(
                        jnzInput{re.to_number(), getNumber_ctre(re.get_end_position(), oneStr.end()).to_number()});
                }
            }
        }
        else if (tmpStr == "tgl") {
    ...
    auto toggleOverload = overloaded{
        ...
        [&](const cpyInput &a) {
            instrVect[togleInstrID] = jnzOffsetTarget{a.valInput_1, a.valInput_2, a.source, a.target};
        },
        ...
        [&](const jnzOffsetTarget &a) {
            instrVect[togleInstrID] = cpyInput{a.valInput_1, a.valInput_2, a.source, a.target};
        },
        [&](const jnzCondOffsetTarget &a) {
            instrVect[togleInstrID] = cpy{a.valInput_1, a.valInput_2, a.source, a.target};
        },
    ...
    while (curInstrID < instrVect.size()) {
        std::visit(
            overloaded{
                ...
                [&](const jnzInput &a) {
                    curInstrID = (a.valInput_1 != 0 ? curInstrID + a.valInput_2 : curInstrID + 1);
                },
                [&](const jnzOffsetTarget &a) {
                    curInstrID = (a.valInput_1 != 0 ? curInstrID + (*a.target) : curInstrID + 1);
                },
                [&](const jnzCondOffsetTarget &a) {
                    curInstrID = (*a.source != 0 ? curInstrID + (*a.target) : curInstrID + 1);
                },
                [&](const tgl &a) {
