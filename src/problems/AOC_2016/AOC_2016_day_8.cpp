...
int day8_1(std::string &&dataFile) {
    ...
    auto ans = flux::ref(disp)
                .fold([] (auto&& init, auto&& a) {
...
std::vector<std::vector<int>> day8_2(std::string &&dataFile) {
    ...
    while (std::getline(iStream, oneStr)) {
        if (oneStr.starts_with("rect")) {
            actionsToTake.push_back(Rect{beforeX_ctre(oneStr.begin(),oneStr.end()).to_number(),afterX_ctre(oneStr.begin(),oneStr.end()).to_number()});
        }
        else if (oneStr.starts_with("rotate row")) {
            actionsToTake.push_back(RotR{afterEquals_ctre(oneStr.begin(),oneStr.end()).to_number(),afterByVal(oneStr.begin(),oneStr.end()).to_number()});
        }
        else if (oneStr.starts_with("rotate column")) {
            actionsToTake.push_back(RotC{afterEquals_ctre(oneStr.begin(),oneStr.end()).to_number(),afterByVal(oneStr.begin(),oneStr.end()).to_number()});
        }
