...
std::string day6_1(std::string &&dataFile) {
    ...
    auto inp =
        incom::aoc::parseInputUsingCTRE::processFile(dataFile, ctreOneChar, ctreOneChar, ctreOneChar, ctreOneChar,
    ...
    for (auto &letterVector : inp) {
        ...
        auto mostCommon = std::ranges::max_element(charCountMap.begin(), charCountMap.end(),
...
std::string day6_2(std::string &&dataFile) {
    ...
    for (auto &letterVector : inp) {
        std::unordered_map<char, int> charCountMap;
        for (auto &letter : letterVector) { charCountMap[letter.front()] += 1; }
        auto mostCommon = std::ranges::min_element(charCountMap.begin(), charCountMap.end(),
                                                   [](auto &&a, auto &&b) { return a.second < b.second; });
        ans.push_back(mostCommon->first);
    }
    return ans;
