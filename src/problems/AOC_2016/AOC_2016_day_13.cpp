...
It is highly probable that the 'location' in queue that is geometrically the closest to the target is more likely to reach it first.
(thus possibly eliminating needless computation of other branches). Possible adverse effect on extreme edge cases.
*/
...
namespace AOC2016 {
    int day13_1(std::string dataFile, int favNumber, const int targetCol, const int targetRow) {
        ...
        auto posIsWall = [favN = favNumber](const int &row, const int &col) {
            ...
            auto isWall = flux::from_range(std::format("{0:b}", val))
                          .count_if([](const auto &a) { return a == '1'; })
                          % 2;
        ...
        for (int i = 0; i < mazeMinDists.size(); ++i) {
            for (int j = 0; j < mazeMinDists[0].size(); ++j) {
                mazeMinDists[i][j].reached = posIsWall(i, j);
            }
        }
