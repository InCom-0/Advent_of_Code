...
int day15_1(std::string &&dataFile) {
    ...
    auto VofV_NumOfPos_StartPos = incom::aoc::parseInputUsingCTRE::processFile(dataFile, getNumOfPos, getStartingPos);
    ...
    auto turnDiscByVal = [](auto &&dsc, const int &val) {
        std::rotate(dsc.position.rbegin(), dsc.position.rbegin() + ((val) % dsc.position.size()), dsc.position.rend());
    };
