...
int day25_1(std::string dataFile) {
    ...
    std::vector<std::variant<cpy, cpyInput, inc, dec, jnz, jnzInput, jnzOffsetTarget, jnzCondOffsetTarget, tgl, skip1,
                             skip2, out, pls>>
        instrVect;
    ...
    while (std::getline(iStream, oneStr)) {
        ...
        if (tmpStr == "cpy") {
            ...
            re = getNumber_ctre(bg, oneStr.end());
            instrVect.push_back(pls{re.to_number(), INT_MIN, nullptr,
                                    mapping.at(getWord_ctre(re.get_end_position(), oneStr.end()).to_string()[0])});
        }
            ...
            std::visit(
                overloaded{
                    ...
                    [&](const out &a) {
                        ...
                        // std::cout << "regState:  " << inputIntoRegA << ' ' << regA << ' ' << regB << ' ' << regC << '
                        // ' << regD << '\n';
                        curInstrID++;
                    ...
                    [&](const pls &a) {
                        *a.target = *a.target + a.valInput_1;
                        curInstrID++;
                    },
