...
int day10_1(std::string &&dataFile, std::pair<int, int> chipToFind) {
    ...
    auto checkBotAddIfToQueue = [&](int &botID) {
        if (botMap[botID].packages.size() == 2 && botMap[botID].lowerDestination != INT_MIN &&
            botMap[botID].higherDestination != INT_MIN) {
            botsToProcess.push(botID);
        }
    };
    ...
    while (std::getline(iStream, oneStr)) {
        ...
        if (goesToBot_ctre(bg, en)) {
            ...
            setDestinationsForBot(botID, lowTargetRgxRes.to_number() + lowMult,
                                  highTargetRgxRes.to_number() + highMult);
        }
    }
    ...
    auto result =
        flux::from_range(botMap)
            .filter([&](auto &&aa) {
                if (flux::ref(aa.second.packages).fold([](auto &&init, auto &&bbb) { return init + bbb; }, 0) ==
                    (chipToFind.first + chipToFind.second)) {
...
int day10_2(std::string &&dataFile, std::pair<int, int> chipToFind) {
    ...
    auto result = flux::from_range(botMap)
                      .filter([&](auto &&aa) { return aa.first >= 1'000'000 && aa.first < 1'000'003; })
