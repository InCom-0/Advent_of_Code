...
int day3_1(std::string dataFile) {
    ...
    auto inp = incom::aoc::parseInputUsingCTRE::processFile(dataFile, ctr, ctr, ctr);
    ...
    auto const ans = flux::zip(flux::ref(inp[0]), flux::ref(inp[1]), flux::ref(inp[2])).count_if([](auto &&a) {
        ...
        int              maxEle = flux::ref(tmpStorage).max([](auto &aa, auto &bb) { return aa < bb; }).value_or(0);
...
int day3_2(std::string dataFile) {
    ...
    return flux::ref(inp).fold(
        [](auto &&z, auto &&y) {
            return z + flux::ref(y).chunk(3).count_if([](auto &&a) {
