        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (mapCopy[ql.rowID + directions[i].first][ql.colID + directions[i].second] == bit::bit1) {
                if (std::abs(ql.rowID + directions[i].first - targetLoc.rowID) > std::abs(ql.rowID - targetLoc.rowID) ||
                    std::abs(ql.colID + directions[i].second - targetLoc.colID) >
                        std::abs(ql.colID - targetLoc.colID)) {
                    curQueue[line + 1].push(day_24_location_t{ql.locID + 1, ql.rowID + directions[i].first,
                                                              ql.colID + directions[i].second});
                }
                else {
                    curQueue[line].push(day_24_location_t{ql.locID + 1, ql.rowID + directions[i].first,
                                                          ql.colID + directions[i].second});
                }
                mapCopy[ql.rowID + directions[i].first][ql.colID + directions[i].second].set(
                    false); // explored positions will not be checked again ever.
            }
        }
        return false;
    };
    ...
    while (std::getline(iStream, oneStr)) {
        ...
        for (auto &letter : oneStr) {
            if (letter >= 48 && letter < 58) {
                locations.push_back({letter - 48, (int)map.size() - 1, (int)map.back().size()});
            }
    ...
    do {
        flux::zip(flux::ref(locations), flux::ref(perm))
            ...
            .for_each([&](auto &&a) {
                ...
                curQueue.front().emplace(day_24_location_t{0, flux::read_at(a, a.first()).first.rowID,
                ...
                while (true) {
                    ...
                    while (not curQueue[line].empty()) {
                        if (routeFinder(flux::read_at(a, sec).first, curQueue[line].front())) {
                            brk = true;
                            break;
                        }
                        curQueue[line].pop();
                ...
                shotestDistances.insert({{flux::read_at(a, a.first()).first.locID, flux::read_at(a, sec).first.locID},
                                         curQueue[line].front().locID});
            });
