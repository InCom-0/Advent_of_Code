...
long long day21_1(std::string dataFile) {
    ...
    auto instructions = incstd::variant_utils::Overloads{
        ...
        [&](jit &a) {
            if (a.m_refs[0].get() != 0) { pqa.m_cursor = (a.m_refs[1] - (a.get_numOfParams() + 1)); }
        },
        [&](jif &a) {
            if (a.m_refs[0].get() == 0) { pqa.m_cursor = (a.m_refs[1] - (a.get_numOfParams() + 1)); }
        },
...
long long day21_2(std::string dataFile) {
    ...
    std::vector<std::string> instrVect;
    instrVect.push_back("OR D J\n");
    instrVect.push_back("OR H T\n");
    instrVect.push_back("OR E T\n");
    instrVect.push_back("AND T J\n");

    instrVect.push_back("AND B T\n");
    instrVect.push_back("AND C T\n");
    instrVect.push_back("NOT T T\n");
    instrVect.push_back("AND T J\n");

    instrVect.push_back("NOT A T\n");
    instrVect.push_back("OR T J\n");
    instrVect.push_back("RUN\n");
    ...
    auto instructions = incstd::variant_utils::Overloads{
        ...
        [&](inp &a) { a.m_refs[0].get() = realInp[inpCursor++]; },
        [&](out &a) {
            // if (a.m_refs[0].get() > 1024) { fmt::print("{}", a.m_refs[0].get()); }
            // else { fmt::print("{}", static_cast<char>(a.m_refs[0].get())); }
            hullDamage = a.m_refs[0].get();
        },
        [&](jit &a) {
            if (a.m_refs[0].get() != 0) { pqa.m_cursor = (a.m_refs[1] - (a.get_numOfParams() + 1)); }
