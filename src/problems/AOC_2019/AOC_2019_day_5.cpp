...
long long day5_1(std::string dataFile, long long inputVal) {
    ...
    auto ol_set = incstd::variant_utils::Overloads{[&](add &a) {}};

    for (auto &str : input.front()) { data.push_back(std::stoll(str)); }
    std::vector<long long> instrCodes{1, 2, 99, 3, 4};

    incom::aoc::PQA::ProgramQuasiAssembly_INT<add, mul, end, inp, out> pqa{instrCodes, data};
...
long long day5_2(std::string dataFile, long long inputVal) {
    ...
    incom::aoc::PQA::ProgramQuasiAssembly_INT<add, mul, end, inp, out, jit, jif, lth, eql> pqa{instrCodes, data};
