...
namespace AOC2017 {
    int day11_1(std::string dataFile) {
        ...
        auto targetCoord = flux::ref(inputVect).map([&] (auto &&a) {return dirMap.at(a);})
                                        .fold([] (auto &&ini, auto &&a) {
                                            ini.first += a.first;
                                            ini.second += a.second;
                                            return ini;
                                        }, std::make_pair(0,0));
    ...
    int day11_2(std::string dataFile) {
        ...
        auto targetCoord = flux::ref(inputVect).map([&] (auto &&a) {return dirMap.at(a);})
                                        .fold([&] (auto &&ini, auto &&a) {
                                            ini.first += a.first;
                                            ini.second += a.second;
                                            ans = std::max(ans, std::abs(ini.second) + (std::max(0, std::abs(ini.first) - std::abs(ini.second))/2));
                                            return ini;
                                        }, std::make_pair(0,0));
        return ans;
    }
