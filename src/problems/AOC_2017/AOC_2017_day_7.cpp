...
namespace AOC2017 {
    std::string day7_1(std::string dataFile) {
        ...
        auto VofV = AOC_commons::parseInputUsingCTRE::processFileRPT(dataFile, wrd_ctre, weight_ctre, discOnTop_ctre);
        std::vector<std::vector<std::string>> VVVVV;

        for (auto &line: VofV) {
            if (line.size() == 3) {
                auto const VofS = AOC_commons::parseInputUsingCTRE::processOneLineRPToneVect(line.back(), wrd_ctre);
                line.pop_back();
                for (auto &itemOnTop: VofS) {
                    line.push_back(itemOnTop);
                }
            }
        }

        std::unordered_map<std::string, int, AOC_commons::XXH3Hasher> UOmap;
        for (auto &line: VofV) {
    ...
    int day7_2(std::string dataFile) {
        ...
        std::unordered_map<std::string, std::pair<std::reference_wrapper<prog>, int>, AOC_commons::XXH3Hasher> mp;
        ...
        * ... impossible to know whether the higher one is the right one or the lower one is. That is decided by the level below.
        *
        * Could probably avoid this by inverting the algo (that is start from the 'leaves' not the 'root') AND using BFS instead of DFS (naturally).
        */
        std::vector<int> ans;
        auto evaluateIncorrectWeight = [&] (this auto& self, prog &prog2Evaluate) -> int {
            std::vector<int> evaResult;
            for (auto &ref2prog: prog2Evaluate.onTop) {
                evaResult.push_back(self(ref2prog.get()));
            }
            ...
            if (evaResult.size() > 1) {
                ...
                if (flux::read_at(flux_evaResult, min) != flux::read_at(flux_evaResult, max)) {
                    if (flux_evaResult.count_if([&, m = flux::read_at(flux_evaResult, min)] (auto &&a) {return a == m;}) == 1) {
                        ...
                        ans.push_back(refToAns.get().myWeight + (flux::read_at(flux_evaResult, max) - flux::read_at(flux_evaResult, min)));
                        refToAns.get().myWeight = flux::read_at(flux_evaResult, max);
                        evaResult[flux::distance(flux_evaResult, flux_evaResult.first(), min)] = flux::read_at(flux_evaResult, max);
                    }
                    else {
                        ...
                        evaResult[flux::distance(flux_evaResult, flux_evaResult.first(), max)] = flux::read_at(flux_evaResult, min);
                    }
                }
            }
        ...
        std::string startName = std::min_element(mp.begin(),mp.end(), [] (auto &&a, auto &&b) {return a.second.second < b.second.second;})->first;
