...
long long day18_1(std::string dataFile) {
    ...
    for (auto &line : VofV) {
        for (long long i = 1; i < line.size(); ++i) {
            if (line[i].front() >= 'a' && line[i].front() <= 'z') {
                // 'Named' registers are inserted long longo a vector named registers + UOmap is created to obtain the
                // right reference later
    ...
    for (auto &line : VofV) {
        ...
        if (line.size() >= 2) {
            if (line[1].front() >= 'a' && line[1].front() <= 'z') { first = mapping.at(line[1][0]); }
                ...
                // Integer inputs in the instructions are treated 'as-if' it were another but 'unnamed' register used
    ...
    auto      overload_obj    = overloaded{
        ...
        [&](const rcv &a) {
            if (a.source != 0) { curInstrID = LLONG_MAX; }
        },
        [&](const jgz &a) {
            if (a.source > 0) { curInstrID += (a.target - 1); }
        },
    };

    // MAIN LOGIC
    while (curInstrID < instrVect.size()) { std::visit(overload_obj, instrVect[curInstrID++]); }
...
long long day18_2(std::string dataFile, int numOfPrograms) {
    ...
    struct prog {
        ...
        prog(decltype(VofV) &input, long long programID) {
            ...
            for (auto &line : input) {
                for (long long i = 1; i < line.size(); ++i) {
                    if (line[i].front() >= 'a' && line[i].front() <= 'z') {
                        // 'Named' registers are inserted long longo a vector named registers + UOmap is created to
    ...
    auto overload_obj  = overloaded{
        ...
        [&](const jgz &a) {
            if (a.source > 0) { curInstrIDs[progID] += (a.target - 1); }
