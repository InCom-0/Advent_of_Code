...
namespace AOC2017 {
    ...
    Right now doing 1000 iterations of the instructions and then repeating the outcome that gives 1'000'000 times ... takes like 15 secs to compute.
    ...
    std::string day16_2(std::string dataFile) {
        ...
        for (int i = 0; i < 1000; ++i) {
            for (auto &oneInstr: posBasedInstr) {
                std::visit(overloaded {
                                        [&] (spn const &a) {std::rotate(theOtherOne.rbegin(), theOtherOne.rbegin()+a.num_1, theOtherOne.rend());},
        ...
        for (int i = 0; i < 1'000'000; ++i) {
            for (int j = 0; j < theOne.size(); ++j) {
                theOtherOne[posBasedMap[j]] = theOne[j];
            }
